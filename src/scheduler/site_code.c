...
site_find_alloc_share(server_info *sinfo, char *name)
{
	...
	if (si && si->pattern_type == PATTERN_SEPARATE &&
	...
	while (fgets(buf, LINE_BUF_SIZE, fp) != NULL) {
		...
		if (strcasecmp(sp, "classes") == 0) {
			...
			while ((sp = strtok_r(NULL, " \t\n", &save_ptr)) != NULL) {
				...
				if (ttype == NULL) {
					ttype = malloc(sizeof(*ttype) + strlen(sp2));
					if (ttype == NULL)
						goto err_out_l;
			...
			++state;
			tshares = malloc(new_cls_cnt * sizeof(*tshares));
			if (tshares == NULL) {
				goto err_out_l;
			}
			continue;
		...
		if (strpbrk(pattern, "|*.\\(){}[]+") != NULL) {
			int result;
			enum pattern_type ptype = PATTERN_COMBINED;
			char *t = malloc(strlen(pattern) + 3);
			char *t2 = pattern;
			if (t != NULL) {
				if (*t2 == '+') {
					ptype = PATTERN_SEPARATE;
					++t2;
				}
				t[0] = '^';
				strcpy(t+1, t2);
