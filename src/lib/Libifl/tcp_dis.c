...
static int tcp_read(int fd)
{
	...
	if (enc->tdis_eod - enc->tdis_lead >= 4) {
		...
		if (enc->tdis_thebuf[enc->tdis_lead]>0x1f) {
		/* old client cleartext */
			gss_buffer_desc *msg_in;
                        msg_in = malloc(sizeof(gss_buffer_desc));

			msg_in->length = enc->tdis_eod - enc->tdis_lead;
			msg_in->value = malloc(msg_in->length);
                        memcpy(msg_in->value, &enc->tdis_thebuf[enc->tdis_lead],msg_in->length);

                        //OM_uint32 minor;
                        //gss_delete_sec_context(&minor, &sec_ctx, GSS_C_NO_BUFFER);
                        //sec_ctx = GSS_C_NO_CONTEXT;
                        DIS_tcp_release(fd);

                        read = tcp_fillbuffer(out,msg_in);
                        return read;
		}
