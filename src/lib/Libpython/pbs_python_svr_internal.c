...
pbs_python_load_python_types(struct python_interpreter_data *interp_data)
{
	...
	if (!(PyPbsV1Module_Obj
		= PyDict_GetItemString(py_sys_modules, PBS_PYTHON_V1_MODULE))) {
		goto ERROR_EXIT;
	}
			...
			for (k=0; k < num_keys; k++) {
				...
				if ((strcmp(resc, WALLTIME_RESC) == 0) && has_resv_duration) {
					snprintf(log_buffer, LOG_BUF_SIZE-1,
						...
						"already specified", resc, ATTR_resv_duration);
					log_buffer[LOG_BUF_SIZE-1] = '\0';
...
pbsv1mod_meth_is_attrib_val_settable(PyObject *self, PyObject *args, PyObject *kwds)
{
	...
	if( (is_resource == 1) && (strcmp(name, PY_RESOURCE_GENERIC_VALUE) == 0) \
			   && (strcmp(resource, PY_RESOURCE_NAME) == 0) ) {
		rc = 0; /* return True */
		goto IAVS_OK_EXIT;
	}
	...
	int opval_len, char *delval, int delval_len)
{
	...
	py_attr_hookset_dict = PyObject_GetAttrString(py_job,
		PY_ATTRIBUTES_HOOK_SET); /* NEW */
		...
		case 1:	/* in __init__ method */
			...
			if (strcmp(obj_name, ITER_QUEUES) == 0) {
				...
				for (vi=0; vi < svr_totnodes; vi++) {
					...
					if( (pbsndlist[vi]->nd_state & INUSE_DELETED) \
							== 0  ) {
						iter_entry->data = \
