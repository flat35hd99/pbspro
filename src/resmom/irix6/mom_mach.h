...
#ifndef MOM_MACH
#define MOM_MACH "irix6"

#define SET_LIMIT_SET   1
#define SET_LIMIT_ALTER 0
#define PBS_CHKPT_MIGRATE 0
#define PBS_PROC_SID(x)  proc_array[x].info.pr_sid
#define PBS_PROC_PID(x)  proc_array[x].info.pr_pid
#define PBS_PROC_PPID(x) proc_array[x].info.pr_ppid
...
struct startjob_rtn {
	...
	jid_t sj_jid;		/* Irix Job container ID */
};
